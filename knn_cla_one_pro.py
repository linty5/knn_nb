        ...
        if row_num!=0:
            ...
            for j in range(0, len(de_la_list)): #将没出现过的单词添入列表
                if de_la_list[j] not in word_list :
                    ...
                    continue
        row_num += 1
    one_hot_matrix = np.zeros((row_num-1, len(word_list)+2))#创建one-hot矩阵
    rocount = 0
    for row2 in train_set_reader2:
        if rocount != 0:
            ...
            for j in range(0, len(de_la_list)):
                ...
                one_hot_matrix[rocount - 1][len(word_list)+1] += 1
        rocount += 1

    np.savetxt(pathnine, one_hot_matrix, fmt="%d", delimiter=" ")  # 将one-hot矩阵写入文件
    return row_num, word_list, one_hot_matrix, emotion_list

def knn_cla():
    socount = 0
    k_test_k = []
    k_test_right = []
    de_one_hot_matrix = np.zeros((row_num-1, len(word_list)+2))  # 创建one-hot矩阵
        ...
        if socount!=0:
            ...
            for j in range(0, len(de_la_list)):
                if de_la_list[j] in word_list:
                    ...
                    de_one_hot_matrix[socount - 1][len(word_list) + 1] += 1
        socount += 1
        ...
        if juli == 1:
            for df in range(0, row_num - 1):
                ...
                for j in range(0, len(word_list)):
                    dsum += abs(de_one_hot_matrix[i][j]-one_hot_matrix[df][j])
                    if de_one_hot_matrix[i][j]!=0 and one_hot_matrix[df][j] != 0:
                        same_count += 1
                if same_count!= 0:
                    dsum = dsum+(de_one_hot_matrix[i][len(word_list)+1]+one_hot_matrix[i][len(word_list)+1])/(same_count*2)
                if same_count == 0:
                    dsum = dsum + 99
                #print(dsum)
                knn_sum.append(dsum)
        ...
        if juli == 2:
            for df in range(0, row_num - 1):
                ...
                for j in range(0, len(word_list)-1):
                    dsum += (de_one_hot_matrix[i][j]-one_hot_matrix[df][j])*(de_one_hot_matrix[i][j]-one_hot_matrix[df][j])
                dsum = dsum ** 0.5
                knn_sum.append(dsum)
                knn_emotion.append(emotion_list[int(one_hot_matrix[df][len(word_list)])])
        if juli == 3:
            for df in range(0, row_num - 1):
                ...
                for j in range(0, len(word_list)):
                    ...
                    if de_one_hot_matrix[i][j]!=0 and one_hot_matrix[df][j] != 0:
                        same_count += 1
                if same_count!= 0:
                    dsum = dsum+(de_one_hot_matrix[i][len(word_list)]+one_hot_matrix[i][len(word_list)])/(same_count*2)
                if same_count == 0:
                    dsum = dsum + 99
                ...
                normA = 0.0
                normB = 0.0
                for a, b in zip(de_one_hot_matrix[i], one_hot_matrix[df]):
                    dot_product += a * b
                    normA += a ** 2
                    normB += b ** 2
                if (normA * normB) == 0:
...
if __name__ == '__main__':
    ...
    row_num, word_list, one_hot_matrix, emotion_list = one_hot()
    juli = 3
    knn_cla()
    end_time = time.time()
